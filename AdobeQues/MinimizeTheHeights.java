...
public class MinimizeTheHeights {
    ...
    // Explnsation on code:
    // 1. We sorted the array first becuase minimum differnece can only be found
    // from consecutive elements

    // 2. After sorting, I have to maximize the (i-1)th element and have to minimize
    // the (i)th element, see inside for loop

    // 3. 'if' condition in loop is added so that if 'min' is negative, the
    // difference max-min will increase and we dont want that.
    // Also it is mentioned in the ques that the resultant height of the tower
    // cannot be negative
